...
interface Auth {
  user: any;
  signUp: (credentials: object) => Promise<object | null>;
  signIn: (credentials: object) => Promise<object | null>;
  signOut: () => void;
}
...
function useProvideAuth(): Auth {
  ...
  //signUp method creates user in DB,
  //if account is valid, update user in state and return user
  const signUp = async (credentials: object): Promise<object | null> => {
    const response = await fetch('http://localhost:3333/auth/signup', {
      method: 'POST',
      headers: {
        'Content-Type': 'Application/JSON'
  ...
  //signin method verifies user
  //if account is valid, update user in state and return user
  const signIn = async (credentials: object): Promise<object | null> => {
    const response = await fetch('http://localhost:3333/auth/login', {
      method: 'POST',
      headers: {
        'Content-Type': 'Application/JSON'
